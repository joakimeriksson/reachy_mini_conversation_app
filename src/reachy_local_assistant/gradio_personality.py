...
class PersonalityUI:
    ...
    def create_components(self) -> None:
        ...
        self.voice_dropdown = gr.Textbox(label="Piper voice (name or .onnx path)", value=config.PIPER_VOICE, interactive=not is_locked)
        ...
        def _read_voice_for(name: str) -> str:
            try:
                ...
                if vf.exists():
                    ...
                    return v or config.PIPER_VOICE
            except Exception:
                pass
            return config.PIPER_VOICE

        def _fetch_voices(selected: str) -> dict[str, Any]:
            """Populate the Piper voice field with the selected profile's voice."""
            return gr.update(value=_read_voice_for(selected))
        ...
        def _new_personality() -> tuple[
            ...
            try:
                ...
                return (
                    ...
                    gr.update(value=config.PIPER_VOICE),
                )
            except Exception:
                return (
                    gr.update(),
